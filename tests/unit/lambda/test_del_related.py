...
def test_del_related_article(theme_repo, mock_context):
...
def test_del_related_theme_not_found(theme_repo, mock_context):
    event = {
        "pathParameters": {
    ...
    response = lambda_handler(event, mock_context, theme_repo, useGlobal=False)

    assert response["statusCode"] == 404
    assert response["body"] == {"message": "Theme not found"}


def test_del_related_success(theme_repo, mock_context):
    event = {
        "pathParameters": {
...
def test_del_related_exception(theme_repo, mock_context):
    event = {
        "pathParameters": {
            "title": "existing_theme",
            "article_id": "1234123-2afdsdf-2323-1234",
